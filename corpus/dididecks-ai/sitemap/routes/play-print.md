...
lede: 'Print-styled route that renders every slot for a given (deck, variant) as a
  stacked sequence of SlideCanvas instances, page-broken with `page-break-after: always`
  per slide. No @page size override — the browser decides paper size; each SlideCanvas
  letterboxes inside that printable area via CSS-only container-query scaling (`cqi`/`cqb`).
  Reader prints via the browser dialog; PDF comes out as one slide per page. Static
  HTML/CSS, no JS — same constraint as Play-UI slides themselves, because runtime
  hydration would break the print render.'
artifact_kind: route
ownership: shell
mode: play-ui
status: shipped
