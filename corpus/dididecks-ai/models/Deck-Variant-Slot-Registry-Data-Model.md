...
lede: 'The spine of dididecks. Each client carries TypeScript modules at src/data/{decks,slides}.ts
  that the shell''s registry-loader evaluates at build time to derive what decks +
  variants + slots exist. The filesystem is the source of truth for slot existence
  — the scroll-page section annotations are scanned and reconciled against the manual
  map. Translating this to a DB means modeling the registry the same way it composes
  today: deck → variants → slots, plus derived runtime state.'
