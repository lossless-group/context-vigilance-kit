...
lede: 'Astro''s `.astro` frontmatter is parsed by esbuild''s TS pipeline before reaching
  the runtime, and the parser sometimes misclassifies `/* */` block comments and certain
  regex-literal shapes when they appear in expression-adjacent positions. The error
  message reports a position past EOF on the affected line, which makes the bug feel
  like a phantom. Workaround: prefer line comments and string methods in `.astro`
  frontmatter where you''d reach for an inline block comment or a slash-heavy regex.'
