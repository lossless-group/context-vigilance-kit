---
slide_number: 8
slug: european-landscape
section: Market
eyebrow: null
headline: European HealthTech VC Landscape
subhead: 'This momentum is attracting capital, but positioning matters; We leverage
  our network to capture value at the super-early-stage.

  '
left_panel:
  title: What is our unique position?
  positioning:
    focus: HealthTech (software)
    stage: Super Early — 60% Pre-Seed, 40% Seed
  matrix_axes:
    y: Stage
    x: Focus
  competing_funds:
  - name: Amino Collective
    country: GER
    size: 60M
  - name: NINA Capital
    country: ESP
    size: 50M
  - name: Civil Bank Ventures (CV)
    country: GER
    size: 100M
  - name: Calm/Storm
    country: AUT
    size: null
    our_fund: true
  - name: Catalpa Ventures
    country: LUX
    size: 30M
  - name: HealthCap
    country: DNK
    size: 120M
  - name: LUMO
    country: NLD
    size: 100M
  - name: YZR
    country: GER
    size: 180M
  - name: Vorwerk Ventures
    country: GER
    size: 150M
  - name: Heal Capital
    country: GER
    size: 120M
  - name: Kurma
    country: FRA
    size: 200M
  - name: Verve Ventures
    country: SUI
    size: 150M
  - name: redalpine
    country: SUI
    size: 200M
right_panel:
  title: What is happening in the market?
  signals:
  - Generalist and multi-stage funds moving into HealthTech.
  - Increasing number of dedicated HealthTech teams.
  - Strong capital inflows signal growing market maturity.
  example_funds:
    early_stage:
    - AlbionVC
    - SEEDCAMP
    - Speedinvest
    - Creandum
    - atomico
    - btov
    - Northzone
    - Octopus
    - Sofinnova partners
    - Lake Star
    - Earlybird
    - General Catalyst
    growth:
    - GHC Healthcare
    - Balderton
    - lightrock
    - EQT
key_stat: 60/40
key_stat_label: Pre-Seed / Seed split
cta_or_footer: 'Note: Below logos indicate the fund''s HQ geography; figures represent
  fund size in EUR (€) — assumed numbers where not public yet. Other highly active
  health VCs in the ecosystem include Firstminute, Norrsken, Karista, Salica, Partech,
  DN Cap, Heartfelt, & others.

  '
sources: []
site_uuid: 34be1236-28ca-4560-aa57-65bca94e8ce0
hex_code: 2ca8lk
date_created: 2026-05-01
publish: true
source_root: /Users/mpstaton/code/lossless-monorepo/ai-labs/dididecks-ai/client-sites/calmstorm-decks/context-v
source_relative_path: narratives/08-european-landscape.md
source_repo_slug: calmstorm-decks
collated_at: '2026-08-18'
source_path: "ai-labs/dididecks-ai/client-sites/calmstorm-decks/context-v/narratives/08-european-landscape.md"
---

# Slide 8 — European HealthTech VC Landscape

## What this slide is

A competitive map of European HealthTech VC. Two panels: **left** is the dedicated-HealthTech-VC quadrant (the field Calm/Storm plays in), with a positioning matrix on Stage × Focus axes. **Right** is the broader market signal — generalist/multi-stage funds entering HealthTech.

## Why it's here

This is the **moat slide**. The LP question it answers: *"Who else does what they do? Why aren't the big funds eating their lunch?"*

The answer is built into the matrix axes:
- **Stage axis (Pre-Seed → Seed → Series A)** — Calm/Storm is at the Pre-Seed/Seed end.
- **Focus axis (Other Health → HealthTech Software)** — Calm/Storm is at the HealthTech-software end.

Their corner of the matrix (super-early × HealthTech-software) has very few competitors. Most named competitors are bigger (€100M+ funds) and at slightly later stages. The implicit message: *to compete with us at our spot, you'd have to be a small, software-focused, Pre-Seed-led fund — and very few of those exist in Europe.*

The right panel functions as **the threat acknowledgment**: yes, generalists are coming into HealthTech, but they're playing a different game (Series A and growth, not Pre-Seed). Acknowledging the threat strengthens the positioning argument; ignoring it would feel naive.

## What's most important to surface

**The Stage × Focus matrix** is the core argument. Without the axes, the logo cluster is just a list of funds. With the axes, the cluster reveals empty space — exactly where Calm/Storm sits.

**The 60% Pre-Seed / 40% Seed positioning** at the top of the left panel is the precise allocation claim. This is the moat language: *we're not "early stage"; we're 60/40 Pre-Seed/Seed*. The precision makes the positioning credible. Visually elevate this line.

**The two panel headers — "What is our unique position?" / "What is happening in the market?"** — are doing the framing. The left/right contrast IS the slide's argument. The two questions belong as visible, equal-weight panel banners.

**The right-panel logo grid** (Atomico, Northzone, Octopus, Earlybird, General Catalyst, EQT, Balderton, etc.) is intimidating in a useful way: it shows Calm/Storm understands these are major players. By pre-naming the threats, they own the conversation; an LP who would have asked "but what about Atomico?" sees Atomico already on the slide.

## Composition risk

This is a chart-like slide. Source uses two boxed panels with a positioning quadrant on the left and a flat logo grid on the right. The chart-axes need to read as axes, not as labels. The cluster of logos with country/size annotations is information-dense — the eye needs gridlines or alignment cues, not just floating logos.

## Visual hierarchy I'd suggest

1. "European HealthTech VC Landscape" headline + subhead
2. Two panel banners — *unique position* / *market signals* — equal weight
3. Calm/Storm's positioning statement (HealthTech focus + 60/40 split) — visually pinned at the top of the left panel
4. **The matrix axes** (Stage / Focus) — labeled
5. Calm/Storm's marker on the matrix — visually distinct from competitors (different border, different fill)
6. Competing fund logos with country + size
7. Right-panel signals (3 bullets, top)
8. Right-panel logo grid (early-stage row + growth row)
9. Footnote with country/size disclaimer + adjacent funds list
