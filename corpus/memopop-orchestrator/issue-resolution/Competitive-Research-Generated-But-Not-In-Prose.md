...
lede: The competitive landscape evaluator agent produces rich, named, evidenced competitor
  data — 9+ companies per memo, with funding, differentiation, and direct/indirect
  classification — and saves it to disk. The downstream writer agent that drafts the
  Category Leadership section never sees that data. The section then emits `<needs-source>`
  markers asking for 'names of the top 3–5 ocean energy competitors' while a file
  naming 9 of them sits two directories away. Two agents working on the same memo
  with no pipe between them.
