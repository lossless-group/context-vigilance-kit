...
lede: 'Chroma has the data — 5,224 context-v chunks plus 257 changelog entries across
  the Lossless tree — but we still load context by hand or by Claude Code dir-scan.
  This spec names the three loops that turn the corpus from a Q&A snapshot into a
  living loading system: an agent that auto-fires the working-set ask at session start,
  a nudge policy that keeps users ambiently aware of recent activity, and a portable
  freshness ceremony so any pseudomonorepo can adopt the same shape.'
