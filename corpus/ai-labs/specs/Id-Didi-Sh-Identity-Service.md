...
lede: 'One small, owned identity service — headless-first API, a signed session cookie
  on .didi.sh, invite-only accounts created from inside whichever app the user arrived
  at. Built on Elixir/Phoenix: the one service in the estate where the implementation
  language is invisible by design, and the BEAM''s reliability posture matches what
  an identity service actually is.'
