---
title: /scroll/pitch/enhanced-v3/ — the most-recent Scroll-UI variant of the Chroma
  pitch deck
lede: Single long page composed of 16 inline `<section data-slot data-variant>` tags
  representing the full pitch in Scroll-UI mode. The active slot is discovered by
  SlideRankPill's IntersectionObserver via the data-attrs. Mounts SlideRankPill directly
  today; target migration is to swap that for `<DeckOverlay--Scroll-UI deckSlug="pitch"
  variantSlug="enhanced-v3" />` so future overlay additions (variant cycling, notes,
